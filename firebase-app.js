console.log('[Firebase] Loading firebase-app.js v20...');
// ============================================
...
// Firebase is initialized inside initFirebase() to avoid duplicate app errors.
...
async function isNickTaken(nick) {
    ...
    try {
        ...
        console.warn('[Firebase] Nick check failed, assuming available:', e);
        return false;
    }
}
...
async function toggleLikeComment(contentOwnerUid, itemId, myUid) {
    if (!db) return;
    const likeRef = db.collection('users').doc(contentOwnerUid)
        .collection('watchData').doc(itemId)
...
async function getCommentLikes(contentOwnerUid, itemId) {
    if (!db) return [];
    const snap = await db.collection('users').doc(contentOwnerUid)
        .collection('watchData').doc(itemId)
        .collection('likes').get();
    return snap.docs.map(d => d.id); // Array of uids who liked
}

async function getFriendActivityFeed(uid) {
    if (!db || !uid) return [];
    ...
    for(const fuid of friendUids) {
        const friendProfile = await getUserProfile(fuid);
        const actionsSnap = await db.collection('users').doc(fuid)
